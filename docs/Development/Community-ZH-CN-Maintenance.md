...
## 2026-09-22：联机可选费用传输失败（报偿邪术）

- 用户报告主机在对方尝试进入战斗时抛出 `GameActionUtil.addOptionalCosts` 的空指针。不能仅凭此堆栈认定用户取消了对话框；主机会执行双方的游戏逻辑。
- 本机 `network-debug-20260922-211656.log` 的 21:21:58 记录显示：战斗开始阶段选择《报偿邪术》后，主机发送 `getChoices` 请求时先发生 `NotSerializableException: forge.game.GameEntityCounterTable`；编码失败返回 null，随后才触发费用列表的空指针。
- `CostBlight` 继承 `CostPutCounter`。后者的 `counterTable` 是主机支付过程中的临时记录，不应该随 `OptionalCostValue` 发给客机。将该字段标为 transient，并在 `readObject` 中重新初始化；不能只标 transient 而让往返后的支付或 `resetLists()` 访问空字段，也不应为了网络发送而把整个游戏对象图改成可序列化。
- `GameActionUtil.addOptionalCosts` 将 null 结果视为失败/取消，返回 null，让已有施放流程在支付费用、移动卡牌之前退出。空列表仍表示明确不支付额外费用，继续使用原咒语；二者不能混同。
- `OptionalCostWireTest` 使用实际的带帧头/LZ4 编码器、解码器和类过滤器，覆盖衰萎、普通指示物和忠诚指示物费用的请求/回复往返，以及临时记录隔离和重新初始化。`OptionalCostSelectionTest` 覆盖 null、空列表、确认支付和远端失败时不继续施放。
- 修复前新增 7 项中 5 项复现错误；修复后与网络反序列化安全、FRA、中文导入、tokens 和启动测试合计 123 项通过。未关闭网络类过滤，也没有修改大厅/中继服务。
- 完整桌面测试包独立输出到 `dist/desktop-cn0922r4-optional-cost-fix/`，暂沿用 cn0922r4 标识，不替换线上同版本制品；主机需要使用修复后的客户端重新开房复测《报偿邪术》的支付/不支付以及战斗阶段推进。

### cn0922r5 发布范围

- 用户要求先提交发布本次修复，《不死涟动》的历史报错留待再次出现时收集日志，不认定为同一原因，也不将其列为已修复。
- 构建标识 `2.0.15-cn0922r5`、桌面显示版本 `2.0.15-汉化-09.22.5`、Android versionCode `2026092205`。公共游戏逻辑的修复同时进入 Windows 和 Android，建议联机双方更新后重新开房。
- 正式版本重新运行相关 123 项测试，零失败、零跳过；固定独立翻译分支与本地 UTF-8 文本按 LF 归一后相同，未引入客户端翻译下载。
